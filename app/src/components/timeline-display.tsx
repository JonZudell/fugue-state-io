...
const TimelineDisplay: React.FC<DisplayProps> = ({ width, height }) => {
  ...
  return (
    ...
    >
      {Object.values(mediaFiles).map((mediaFile, index) => (
        <div
          key={`${index}-div`}
          style={{ marginLeft: (mediaFile.offset / timelineDuration) * width }}
        >
          <TimelineWaveformDisplay
            key={`${index}-timeline`}
            width={Math.max(0, (mediaFile.duration / timelineDuration) * width)}
            height={
              height /
              (Object.keys(mediaFiles).length + Object.keys(abcs).length)
            }
      ...
      {Object.values(abcs).map((abc, index) => (
        <div
          key={`${index}-div`}
          style={{ marginLeft: (abc.offset / timelineDuration) * width }}
        >
          <TimelineNotationDisplay
            key={`${index}-timeline`}
            width={Math.max(0, (abc.duration / timelineDuration) * width)}
            height={
              height /
              (Object.keys(mediaFiles).length + Object.keys(abcs).length)
            }
