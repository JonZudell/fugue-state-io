...
import { faGear, faUser, faMusic, faCode, faSliders, faPen, IconDefinition } from '@fortawesome/free-solid-svg-icons';
...
interface LeftMenuProps {
  smallestSize?: number;
  initialState?: Array<{ id: number; icon: IconDefinition; style: React.CSSProperties; tabContent: JSX.Element }>;
}
...
const LeftMenu: React.FC<LeftMenuProps> = ({ smallestSize = 128, initialState = [
  ...
  return (
    <>
      <div className='menu-height bg-gray-800 w-16 float-left flex flex-col justify-between'>
        <ul>
          {initialState.map((item) => (
            <li key={item.id} className={`icon-button  icon-button:hover ${activeTab === item.id ? 'active' : ''}`}
              onClick={() => handleTabClick(item.id)}>
              <FontAwesomeIcon style={item.style} className={`w-8 h-8 ${activeTab === item.id ? 'offset-margin' : 'm-4'}`} icon={item.icon} />
            </li>
          ))}        </ul>
      ...
      <div>
        {width > smallestSize ? (
          <div style={{ width: `${Math.max(width, smallestSize / 2)}px` }}
        ...
        <div style={{ width: width > smallestSize ? '4px' : '0px', cursor: 'ew-resize' }}
