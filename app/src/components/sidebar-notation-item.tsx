...
import { ABCAsset, MediaFile, selectPlayback, selectProject, setMediaMode, setMediaOffset, setMediaVolume, setNotationOffset, setPrimarySourceId, setReferenceFile } from "@/store/project-slice";
...
export function SidebarNotationItem({ className, abcFile }: SidebarNotationItemProps) {
  ...
  const updateOffset = (value: string) => {
    if (!isNaN(parseFloat(value))) {
      dispatch(setNotationOffset({ id: abcFile.id, offset: parseFloat(value) }));
  ...
  return (
    <Card className={`flex flex-col flex-1 mx-auto text-white p-2 text-sm w-full ${className}`} style={{ overflow: "hidden" }}>
      {abcFile.name}
      ...
      <Input step="0.1" className={`py-2 ${offsetError ? "border-red-500" : "border-white"}`} type="number" defaultValue={abcFile.offset} onChange={(event: any) => { updateOffset(event.target.value)}} />
    </Card>
  );
}
